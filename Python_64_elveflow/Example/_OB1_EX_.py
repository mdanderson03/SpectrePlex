...
while repeat:
    answer = input(
        'what to do (set_p, get_p, get_sens, get_all, get_trig, set_trig, add_pid, start, stop, set_target, read_channel, or exit) : ')
    if answer == 'set_p':
    ...
    if answer == "get_all":
        data_sens = c_double()
        get_pressure = c_double()
        error = OB1_Get_Press(Instr_ID.value, 1, 1, byref(Calib), byref(get_pressure),
                              1000)  # Ch=1;  Acquire_data=1 -> read all the analog values
        print('error: ', error)
        print('Press ch 1: ', get_pressure.value)

        for i in range(2, 5):
            error = OB1_Get_Press(Instr_ID.value, i, 0, byref(Calib), byref(get_pressure),
                                  1000)  # Ch=i;  Acquire_data=0 -> use the values acquired in OB1_Get_Press
            print('error: ', error)
            print('Press ch ', i, ': ', get_pressure.value)

        for i in range(1, 5):
            error = OB1_Get_Sens_Data(Instr_ID.value, i, 0, byref(
                data_sens))  # Ch=i;  Acquire_data=0 -> use the values acquired in OB1_Get_Press
            print('error: ', error)
