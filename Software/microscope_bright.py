...
class cycif:
    ...
    def auto_exposure(self, experiment_directory, x_frame_size, percentage_cut_off=0.999, target_percentage=0.1):
        ...
        for channel in channels:
            ...
            high_pixel, low_pixel = self.image_percentile_level(non_zero_thresh_pixels,
            ...
            print('channel', channel, 'thresh', thresh, 'high', high_pixel, 'low', low_pixel, 'new exp', new_exp_factor,
                  'normalized exp', lowest_exp)
    ...
    def hdr_manual_compression(self, experiment_directory, cycle_number):
        ...
        for channel in channels:
            ...
            low_col = high_col + 1
            tile_directory = experiment_directory + '//' + str(channel) + r'\Stain\cy_' + str(
                cycle_number) + r'\Tiles\focused_subbed_basic_corrected'
    ...
    def hdr_compression(self, experiment_directory, cycle_number, apply_2_subbed=1, apply_2_bleached=1,
                        apply_2_focused=1, apply_2_flattened=1, channels=['DAPI', 'A488', 'A555', 'A647']):
        '''
        ...
        for channel in channels:
            for type in types:
                if type == 'Stain':
                    type_path = experiment_directory + r'//' + channel + r'//' + type + r'//cy_' + str(
                        cycle_number) + r'//Tiles'
                    os.chdir(type_path)
                elif type == 'Bleach':
                    type_path = experiment_directory + r'//' + channel + r'//' + type + r'//cy_' + str(
                        cycle_number - 1) + r'//Tiles'
                    os.chdir(type_path)
                elif type == 'bleach_focused':
                    type_path = experiment_directory + r'//' + channel + r'//' + 'Bleach' + r'//cy_' + str(
                        cycle_number - 1) + r'//Tiles/focused'
                    os.chdir(type_path)
                elif channel == 'DAPI' and type == 'focused_subbed':
                    pass
                elif channel == 'DAPI' and type == 'focused_subbed_basic_corrected':
                    type_path = experiment_directory + r'//' + channel + r'//Stain//cy_' + str(
                        cycle_number) + r'//Tiles/' + 'focused_basic_corrected'
                    os.chdir(type_path)
                else:
                    type_path = experiment_directory + r'//' + channel + r'//Stain//cy_' + str(
                        cycle_number) + r'//Tiles/' + type
                    os.chdir(type_path)
                ...
                if type == 'Stain':
                        ...
                        for y in range(0, y_tile_count):
                            if tissue_fm[y][x] > 1:
                                for z in range(0, z_tiles):
                                    file_name = r'z_' + str(z) + '_x' + str(x) + '_y_' + str(
                                        y) + '_c_' + channel + '.tif'
                    ...
                    if apply_2_flattened == 1:
                        if channel == 'DAPI':
                            ff_path = experiment_directory + r'//' + channel + r'//Stain//cy_' + str(
                                cycle_number) + r'//Tiles/' + 'focused_basic_corrected'
                            os.chdir(ff_path)
                        else:
                            ff_path = experiment_directory + r'//' + channel + r'//Stain//cy_' + str(
    ...
    def tissue_cluster_filter(self, experiment_directory, x_frame_size, number_clusters_retained=1, area_threshold=0.1):
        ...
        new_labelled_image[np.nonzero(new_labelled_image)] = new_labelled_image[np.nonzero(new_labelled_image)] - (
                    65535 - number_actual_clusters_retained)
    ...
    def recursive_stardist_autofocus(self, experiment_directory, cycle, slice_gap=2, remake_nuc_binary=0):
        ...
        z_middle = math.floor(
            z_count / 2)  # if odd, z_count will round down. Since index counts from 0, it is the middle
    ...
    def reacquire_run_autofocus(self, experiment_directory, cycle, z_slices, offset_array, x_frame_size):
        ...
        self.image_cycle_acquire(cycle, experiment_directory, z_slices, 'Stain', offset_array,
                                 x_frame_size=x_frame_size, establish_fm_array=0, auto_focus_run=0, auto_expose_run=0,
                                 channels=['DAPI'])
    ...
    def image_cycle_acquire(self, cycle_number, experiment_directory, z_slices, stain_bleach, offset_array,
                            x_frame_size=5056, fm_array_adjuster=0, establish_fm_array=0, auto_focus_run=0,
                            auto_expose_run=0,
        ...
        self.establish_fm_array(experiment_directory, cycle_number, z_slices, offset_array,
                                initialize=establish_fm_array, x_frame_size=x_frame_size,
                                fm_array_adjuster=fm_array_adjuster, autofocus=auto_focus_run,
        ...
        start = time.time()
        self.multi_channel_z_stack_capture(experiment_directory, cycle_number, stain_bleach, x_pixels=x_frame_size,
                                           slice_gap=2, channels=channels)
        end = time.time()
        print('acquistion time', end - start)
        # self.marker_excel_file_generation(experiment_directory, cycle_number)

    def wide_net_auto_focus(self, experiment_directory, x_frame_size, offset_array, z_slices, focus_position,
                            number_clusters_retained=6):
    ...
    def initialize(self, experiment_directory, offset_array, z_slices, x_frame_size=2960, focus_position='none',
        ...
        self.wide_net_auto_focus(experiment_directory, x_frame_size, offset_array, z_slices, focus_position,
                                 number_clusters_retained=number_clusters)
        ...
        self.image_cycle_acquire(0, experiment_directory, z_slices, 'Bleach', offset_array, x_frame_size=x_frame_size,
                                 fm_array_adjuster=0, establish_fm_array=0, auto_focus_run=0, auto_expose_run=3,
                                 focus_position=focus_position)
        # self.generate_nuc_mask(experiment_directory, cycle_number=0)
        # self.tissue_region_identifier(experiment_directory, clusters_retained=4)

    def full_cycle(self, experiment_directory, cycle_number, offset_array, stain_valve, fluidics_object, z_slices,
                   incub_val=45, x_frame_size=2960, focus_position='none'):

        pump = fluidics_object

        if cycle_number == 0:
            self.initialize(experiment_directory, offset_array, z_slices, x_frame_size=x_frame_size,
                            focus_position=focus_position)
        else:

            # print(status_str)
            print('cycle', cycle_number)
            pump.liquid_action('Stain', incub_val=incub_val, stain_valve=stain_valve, microscope_object=self,
                               experiment_directory=experiment_directory)  # nuc is valve=7, pbs valve=8, bleach valve=1 (action, stain_valve, heater state (off = 0, on = 1))
            # self.reacquire_run_autofocus(experiment_directory, cycle_number, z_slices, offset_array, x_frame_size)
            # print(status_str)
            self.image_cycle_acquire(cycle_number, experiment_directory, z_slices, 'Stain', offset_array,
                                     x_frame_size=x_frame_size, establish_fm_array=0, auto_focus_run=0,
                                     auto_expose_run=3)
            time.sleep(5)

            # print(status_str)
            pump.liquid_action('Bleach',
                               stain_valve=stain_valve)  # nuc is valve=7, pbs valve=8, bleach valve=1 (action, stain_valve, heater state (off = 0, on = 1))
            time.sleep(5)
            # print(status_str)
            self.image_cycle_acquire(cycle_number, experiment_directory, z_slices, 'Bleach', offset_array,
                                     x_frame_size=x_frame_size, establish_fm_array=0, auto_focus_run=0,
                                     auto_expose_run=3)
    ...
    def mcmicro_image_stack_generator_separate_clusters(self, cycle_number, experiment_directory, x_frame_size):
        ...
        for x in range(0, number_clusters):
            ...
            xml_metadata = self.metadata_generator_separate_clusters(experiment_directory, x_frame_size,
                                                                     cluster_number=x + 1)
    ...
    def illumination_flattening(self, experiment_directory, cycle_number, rolling_ball=0):
        ...
        for channel in range(2, 3):
            ...
            if channel == 0:
                ...
                training_directory = directory_start + channel_name + '\Stain\cy_' + str(
                    cycle_number) + r'\Tiles\focused'
                stain_output_directory = directory_start + channel_name + '\Stain\cy_' + str(
                    cycle_number) + r'\Tiles\focused_basic_corrected'

            else:
                if rolling_ball != 1:
                    stain_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused'
                    training_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused\temp'
                    stain_output_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused_subbed_basic_corrected'

                if rolling_ball == 1:
                    directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused\background_subbed_rolling'
            ...
            for cluster in range(1, 7):
                for x in range(0, x_tile_count):
                    for y in range(0, y_tile_count):
                        if cluster in self.tissue_fm_decode(tissue_fm[y][x]):
                        #if tissue_fm[y][x] > 2:
                            os.chdir(stain_directory)
                            filename = 'x' + str(x) + '_y_' + str(y) + '_c_'+ channel_name + '.tif'
                ...
                optimizer.directory = training_directory
        ...
        for channel in range(0, 4):
            ...
            if channel == 0:
                if hdr_sub == 0:
                    stain_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused'
                    stain_temp_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused\temp'
                    bleach_directory = directory_start + channel_name + '\Bleach\cy_' + str(
                        cycle_number) + r'\Tiles\focused'
                if hdr_sub == 1:
                    stain_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\subbed_focused'
                    stain_temp_directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\subbed_focused\temp'
            else:
                if rolling_ball != 1:
                    if hdr_sub == 0:
                        stain_directory = directory_start + channel_name + '\Stain\cy_' + str(
                            cycle_number) + r'\Tiles\background_subbed'
                        stain_temp_directory = directory_start + channel_name + '\Stain\cy_' + str(
                            cycle_number) + r'\Tiles\background_subbed\temp'
                        bleach_directory = directory_start + channel_name + '\Bleach\cy_' + str(
                            cycle_number) + r'\Tiles\focused'
                    if hdr_sub == 1:
                        stain_directory = directory_start + channel_name + '\Stain\cy_' + str(
                ...
                if rolling_ball == 1:
                    directory = directory_start + channel_name + '\Stain\cy_' + str(
                        cycle_number) + r'\Tiles\focused\background_subbed_rolling'
            stain_output_directory = directory_start + channel_name + '\Stain\cy_' + str(
                cycle_number) + r'\Tiles\subbed_focused_basic_corrected'
    ...
    def in_focus_excel_populate(self, experiment_directory, cycle_number, x_frame_size, x_sub_section_count=1,
                                y_sub_section_count=1):
            ...
            for x in range(0, x_tile_count):
                for y in range(0, y_tile_count):
                    if tissue_exist[y][x] == 1:
                        ...
                        self.image_reconstructor(experiment_directory, slice_number, channel, cycle_number,
                                                 x_frame_size, y, x)
    ...
    def image_reconstructor(self, experiment_directory, reconstruct_array, channel, cycle_number, x_frame_size,
        ...
        for type in cycle_types:
            ...
            if type == 'Stain':
                ...
                os.chdir(im_path)
                saving_path = experiment_directory + '/' + channel + '/' + type + '\cy_' + str(
            ...
            file_name = 'z_' + str(z_slice) + '_x' + str(x_tile_number) + '_y_' + str(y_tile_number) + '_c_' + str(
                channel) + '.tif'
            image = tf.imread(file_name)
                ...
                if tissue_exist[y][x] == 1:
                    ...
                    if rolling_ball != 1:
                        ...
                        for channel in channels:
                            ...
                            bleach_color_path = experiment_directory + channel + r'/Bleach/cy_' + str(
                                cycle) + '\Tiles/focused'
    ...
    def brightness_uniformer(self, experiment_directory, cycle_number, cluster_number):
        ...
        step_count = 40
        multiplier_mod_values = np.linspace(0.7, 1.3, step_count)
        ...
        for channel in channels:
            ...
            dapi_file_path = experiment_directory + '/' + 'DAPI' + '/Stain/cy_' + str(cycle_number) + '/Tiles/focused_basic_corrected'
            ...
            for x in range(0, x_tile_count):
                for y in range(0, y_tile_count):
                    ...
                    if cluster_number in tile_list:
                        ...
                        overlap_images[y + 1][x + 1][1][0] = dapi_im[::, -x_width -1:-1]
                        overlap_images[y + 1][x + 1][2][0] = np.transpose(dapi_im[-y_width-1:-1, ::])
                        overlap_images[y + 1][x + 1][3][0] = dapi_im[::, 0:x_width]

                        overlap_images[y + 1][x + 1][0][1] = np.transpose(image[0:y_width, ::])
                        overlap_images[y + 1][x + 1][1][1] = image[::, -x_width-1:-1]
                        overlap_images[y + 1][x + 1][2][1] = np.transpose(image[-y_width-1:-1, ::])
            ...
            net_score = np.zeros(iteration_count)
            iteration_axis = np.linspace(1,iteration_count,iteration_count)
                    ...
                    for y in range(0, y_tile_count):
                        if y != -1:
                            ...
                            if cluster_number in tile_list:
                                ...
                                #tile_north = overlap_images[y + 1][x + 1][0][1]
                                #tile_east = overlap_images[y + 1][x + 1][1][1]
                                #tile_south = overlap_images[y + 1][x + 1][2][1]
                                #tile_west = overlap_images[y + 1][x + 1][3][1]


                                #adjacent_north = overlap_images[y][x + 1][0][1]
                                #adjacent_east = overlap_images[y + 1][x + 2][1][1]
                                #adjacent_south = overlap_images[y + 2][x + 1][2][1]
                                #adjacent_west = overlap_images[y][x + 1][3][1]
                                ...
                                adjacent_north = np.multiply(overlap_images[y][x + 1][2][1], multiplier_array[y][x + 1])
                                adjacent_east = np.multiply(overlap_images[y + 1][x + 2][3][1],multiplier_array[y + 1][x + 2])
                                adjacent_south = np.multiply(overlap_images[y + 2][x + 1][0][1],multiplier_array[y + 2][x + 1])
                                adjacent_west = np.multiply(overlap_images[y + 1][x][1][1], multiplier_array[y + 1][x])



                                #print('y', y, 'x', x, np.mean(tile_south), np.mean(adjacent_north))

                                scores = np.zeros(step_count).astype('float64')

                                #determine differences

                                mod_step_size = multiplier_mod_values[0] - multiplier_mod_values[1]

                                north_sum = np.mean(tile_north)
                                east_sum = np.mean(tile_east)
                                south_sum = np.mean(tile_south)
                                west_sum = np.mean(tile_west)

                                mod_north_unit_difference = np.mean(mod_step_size * tile_north - adjacent_north) * tile_exist[y][x + 1]
                                mod_east_unit_difference = np.mean(mod_step_size * tile_east - adjacent_east) * tile_exist[y + 1][x + 2]
                                mod_south_unit_difference = np.mean(mod_step_size * tile_south - adjacent_south) * tile_exist[y + 2][x + 1]
                                mod_west_unit_difference = np.mean(mod_step_size * tile_west - adjacent_west) * tile_exist[y + 1][x]


                                for m in range(0, step_count):
                                    modded_multiplier = multiplier_mod_values[m]

                                    mod_north_component = mod_north_unit_difference + (modded_multiplier - mod_step_size) * north_sum
                                    mod_east_component = mod_east_unit_difference + (modded_multiplier - mod_step_size) * east_sum
            ...
            for y in range(0, y_tile_count):
                for x in range(0, x_tile_count):
                    if tissue_exist[y][x] == 1:
                        ...
                        # save projection
                        saving_path = experiment_directory + '/' + channel + '/Stain' + '/cy_' + str(
            ...
            for x in range(0, x_tile_count):
                for y in range(0, y_tile_count):
                    ...
                    if tissue_exist[y][x] == 1:
                        ...
                        image_path = experiment_directory + '/' + channel + '/Stain' + '/cy_' + str(
                            cycle_number) + '/Tiles' + '/focused'
                        os.chdir(image_path)
    ...
    def antibody_kinetics(self, experiment_directory, capture_rate_staining, duration_staining, stain_valve,
                          fluidic_object, channels=['DAPI', 'A488', 'A555', 'A647']):
